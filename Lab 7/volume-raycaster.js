...
var volumes = {
	"Boston Teapot": "raw/boston_teapot_256x256x178_uint8.raw",
	"Engine": "raw/engine_256x256x128_uint8.raw",
	"Bonsai": "raw/bonsai_256x256x256_uint8.raw",
	"Foot": "raw/foot_256x256x256_uint8.raw"
};
...
var selectVolume = function() {
	...
	loadVolume(volumes[selection], function(file, dataBuffer) {
		...
		console.log(volDims)
		let index = 0;
		for(var x = 0;x<volDims[0];x++)
		{
			for(var y = 0;y<volDims[1];y++)
			{
				for(var z = 0;z<volDims[2];z++)
				{
					if((x-127.5)*(x-127.5)+(y-127.5)*(y-127.5)<=10000)
					{
						dataBuffer[index]=100;
					}
					else
					{
						dataBuffer[index]=0;
